...
def load_and_prepare_data(filepath):
    ...
    # pre-merge data
    filtered_data = data[data['block_date'] <= pd.Timestamp('2022-09-01')]
...
def plot_data(data, filepath):
    ...
    line1, = ax1.plot(data['block_date'], data['private_tx_count'], linestyle='-', color='navy', label='Private Transactions')
    line2, = ax1.plot(data['block_date'], data['flashbots_bundle_count'], linestyle='-', color='limegreen', label='Flashbots Bundles')
