...
def plot_data(data, filepath):
    ...
    ax2 = ax1.twinx()
    line1, = ax1.plot(data['block_date'], data['private_tx_count'], linestyle='-', color='navy', label='Blocknative Private Transactions')
    line2, = ax2.plot(data['block_date'], data['fb_bundle_count'], linestyle='-', color='limegreen', label='Flashbots Bundles')

    ax1.set_xlabel('Date')
    ax1.set_ylabel('Block Median Count', color='navy')
    ax2.set_ylabel('Count', color='limegreen')

    ax1.tick_params(axis='y', labelcolor='navy')
    ax2.tick_params(axis='y', labelcolor='limegreen')

    plt.title('Private Transaction Block Median vs Bundle Count per Day')
    ax1.set_xlim(left=data['block_date'].min())
    ax1.set_ylim(0)
    ax2.set_ylim(0)
    ...
    # Adjust the legend position and set zorder
