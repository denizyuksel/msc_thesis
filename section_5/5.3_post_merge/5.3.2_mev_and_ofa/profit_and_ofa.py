...
def plot_data(data, filepath):
    ...
    ax1.plot(data['block_date'], data['mev_share_count'], color='blue', label='MEV-Share Transactions')
    ...
    plt.title('MEV-Share and MEV Transactions with Extractor Profits')
    ...
    significant_dates = {
        ...
        '2023-03-11': ('darkcyan', '--', 'USDC Depeg Date (Mar 2023)'),
        '2023-04-01': ('purple', '-.', 'MEV Share Launch Date (Apr 2023)'),
        '2023-04-27': ('crimson', '-.', 'MEV Blocker Launch Date (Apr 2023)')
    }
...
def calculate_correlation(data):
    ...
    pearson_corr, pearson_p_value = pearsonr(data.dropna()['mev_share_count'], data.dropna()['mev_tx_count'])
    spearman_corr, spearman_p_value = spearmanr(data.dropna()['mev_share_count'], data.dropna()['mev_tx_count'])
...
def main():
    ...
    plot_data(data_by_date, '5.3.2_profit_mev_ofa.png')
