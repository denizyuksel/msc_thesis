...
aggregated_data['rolling_mev_tx_count'] = aggregated_data['mev_tx_count'].rolling(window=14).mean()

# Calculate Pearson and Spearman correlation coefficients on the raw data
pearson_corr, pearson_p_value = pearsonr(aggregated_data.dropna()['rolling_extractor_profit'], aggregated_data.dropna()['rolling_bundle_tx'])
spearman_corr, spearman_p_value = spearmanr(aggregated_data.dropna()['rolling_extractor_profit'], aggregated_data.dropna()['rolling_bundle_tx'])
...
plt.figtext(0.1, 0.05, f'Pearson Corr (profit vs bundle): {pearson_corr:.2f}, p-value: {pearson_p_value:.3f}', horizontalalignment='left', color='black', fontsize='x-small')
plt.figtext(0.1, 0.02, f'Spearman Corr (profit vs bundle): {spearman_corr:.2f}, p-value: {spearman_p_value:.3f}', horizontalalignment='left', color='black', fontsize='x-small')
...
# Plotting
ax.plot(aggregated_data['block_date'], aggregated_data['rolling_bundle_tx'], color='skyblue', label='Bundle Count per Day')
ax.set_xlabel('Date')
...
ax2 = ax.twinx()
ax2.plot(aggregated_data['block_date'], aggregated_data['rolling_extractor_profit'], color='green', label='Total Extractor Profit per Day')
ax2.set_ylabel('USD', color='green')
ax2.tick_params(axis='y', labelcolor='green')

plt.title('Bundle Counts vs Extractor Profits Before the Merge')
...
plt.savefig('figures/private_mev_profit_bundle.png')  # Make sure the directory exists
# plt.show()
