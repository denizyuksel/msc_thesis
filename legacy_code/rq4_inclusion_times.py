...
aggregated_data['mean_timepending'] = aggregated_data['mean_timepending'].rolling(window=14).mean()

# Calculate Pearson and Spearman correlation coefficients on the raw data
pearson_corr, pearson_p_value = pearsonr(aggregated_data.dropna()['private_gasused_pct'], aggregated_data.dropna()['mean_timepending'])
spearman_corr, spearman_p_value = spearmanr(aggregated_data.dropna()['private_gasused_pct'], aggregated_data.dropna()['mean_timepending'])
...
# Plot private mean gas used
ax1.set_xlabel('Date')
ax1.set_ylabel('Percentage', color='blue')
ax1.plot(aggregated_data['block_date'], aggregated_data['private_gasused_pct'], color='blue', label='Private Gasused')
ax1.tick_params(axis='y', labelcolor='blue')

# Create a second y-axis to plot the mean time pending
ax2 = ax1.twinx()
ax2.set_ylabel('Time (ms)', color='green')
ax2.plot(aggregated_data['block_date'], aggregated_data['mean_timepending'], color='green', linestyle='--', label='Mean Inclusion Time')
...
plt.title('Relationship Between Private Gasused Percentage and Mean Inclusion Time Over Time')
plt.grid(True)
...
plt.figtext(0.1, 0.05, f'Pearson Corr (private gasused pct vs inclusion time): {pearson_corr:.2f}, p-value: {pearson_p_value:.3f}', horizontalalignment='left', color='black', fontsize='x-small')
plt.figtext(0.1, 0.02, f'Spearman Corr (private gasused pct vs inclusion time): {spearman_corr:.2f}, p-value: {spearman_p_value:.3f}', horizontalalignment='left', color='black', fontsize='x-small')
...
plt.savefig('figures/rq4_inclusion_times_gasused_mean.png', bbox_inches='tight')
plt.show()
