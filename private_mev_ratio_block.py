...
pearson_corr, pearson_p_value = pearsonr(aggregated_data.dropna()['rolling_private_tx_pct'], aggregated_data.dropna()['rolling_mev_tx_pct'])
spearman_corr, spearman_p_value = spearmanr(aggregated_data.dropna()['rolling_private_tx_pct'], aggregated_data.dropna()['rolling_mev_tx_pct'])

# Plotting
fig, ax1 = plt.subplots(figsize=(8, 4))
plt.title('Daily Average Percentage of Private & MEV Transactions')
ax1.fill_between(aggregated_data['block_date'], 0, aggregated_data['rolling_private_tx_pct'], color='skyblue', alpha=0.4, label='Average Private Transactions (%)')
ax1.set_xlabel('Date')
...
ax1.annotate(f'Pearson Corr (private vs bundle): {pearson_corr:.2f}, p-value: {pearson_p_value:.3f}', xy=(0.0, -0.2), xycoords='axes fraction', verticalalignment='top', horizontalalignment='left', color='black')
ax1.annotate(f'Spearman Corr (private vs bundle): {spearman_corr:.2f}, p-value: {spearman_p_value:.3f}', xy=(0.0, -0.3), xycoords='axes fraction', verticalalignment='top', horizontalalignment='left', color='black')
