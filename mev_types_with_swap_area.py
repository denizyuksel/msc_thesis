...
aggregated_data['mev_tx_count'] = aggregated_data[['arb_count', 'frontrun_count', 'sandwich_count', 'backrun_count', 'liquid_count', 'swap_count']].sum(axis=1)

# Calculate percentages for each transaction type relative to MEV transactions
transaction_types = ['arb_count', 'frontrun_count', 'sandwich_count', 'backrun_count', 'liquid_count', 'swap_count']
...
plt.title('Percentage of Each MEV Transaction (Swap Included) Type Over Time')
plt.xlabel('Date')
...
plt.legend()
plt.tight_layout()
plt.savefig('figures/mev_types_with_swap_area.png')
# plt.show()